...
public class LdaGibbsSampler {
	...
	public void lda_run(ArrayList<int[]> indexTermArray, int vocabularySize,
			int topicNum){

		/*
		 * Get data.
		 */
		int[][] documents = new int[indexTermArray.size()][];
		for (int i = 0; i < indexTermArray.size(); i++) {
			documents[i] = indexTermArray.get(i);
		}
		...
		configure(1000, 800, 20, 10);
		gibbs(K, alpha, gamma);

		//theta = getTheta();
		//phi = getPhi();
		//sentence2topicCount = nd;
	}
